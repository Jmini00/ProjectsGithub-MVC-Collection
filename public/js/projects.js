 ...
 */
fetch('/ProjectsGithub_MVC-Collection/api/all/projects')
...
document.querySelector('#search').addEventListener('input', function() {
    ...
    fetch(`/ProjectsGithub_MVC-Collection/api/search?query=${searchTerm}`)
        .then(response => response.json())
        .then(projects => {
            // Vider la liste des projets sur l'accueil
            document.querySelector('#vinyles-grid').textContent = ''
...
{
    fetch(`/ProjectsGithub_MVC-Collection/api/project?id=${idProject}`)
        .then(response => response.json())
        .then(project => {
            ...
            document.querySelector('h2').textContent = 'Projet'
            ...
            history.pushState('', '', `/projet/details?id=${idProject}`)
        })
...
function createCard(project, onlyOneProject = false)
{
    ...
    article.querySelector('small').textContent = `Ajouté le ${date.toLocaleDateString('fr')}`
    ...
    document.querySelector('#vinyles-grid').appendChild(article)
}

function isLove(idProject)
{
    fetch(`/ProjectsGithub_MVC-Collection/api/love?id=${idProject}`)
        .then(response => response.json())
        .then(() => {
